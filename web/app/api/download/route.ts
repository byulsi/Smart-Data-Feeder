...
export async function GET(request: Request) {
  ...
  if (type === 'overview') {
    ...
    const { data: financials } = await supabase.from('financials').select('*').eq('ticker', ticker).order('year', { ascending: false }).limit(4)
    const { data: disclosures } = await supabase.from('disclosures').select('*').eq('ticker', ticker).order('rcept_dt', { ascending: false }).limit(10)
    ...
    return new NextResponse(md, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="${company.name}_Overview.md"`
      }
    })

  } else if (type === 'narratives') {
    // Placeholder for Narratives
    const { data: company } = await supabase.from('companies').select('name').eq('ticker', ticker).single()
    const { data: disclosures } = await supabase.from('disclosures').select('*').eq('ticker', ticker).order('rcept_dt', { ascending: false }).limit(10)
    ...
    return new NextResponse(md, {
      headers: {
        'Content-Type': 'text/markdown; charset=utf-8',
        'Content-Disposition': `attachment; filename="${company.name}_Narratives.md"`
      }
    })
    ...
    return new NextResponse(header + rows, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="${ticker}_Chart.csv"`
      }
    })
  }
